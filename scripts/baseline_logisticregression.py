...
def intersect_modified(list1, list2):
    ...
    union = cnt1 | cnt2
    largeinter = Counter()
    for (element, count) in list(inter.items()):
...
def paraphrase_Das_features(source, target, trend):
    ...
    if (precision1gram + recall1gram) > 0:
        f1gram = 2 * precision1gram * recall1gram / \
            (precision1gram + recall1gram)
    ...
    if (precision2gram + recall2gram) > 0:
        f2gram = 2 * precision1gram * recall2gram / \
            (precision2gram + recall2gram)
    ...
    if (precision3gram + recall3gram) > 0:
        f3gram = 2 * precision3gram * recall3gram / \
            (precision3gram + recall3gram)
    ...
    if (precision1stem + recall1stem) > 0:
        f1stem = 2 * precision1stem * recall1stem / \
            (precision1stem + recall1stem)
    ...
    if (precision2stem + recall2stem) > 0:
        f2stem = 2 * precision2stem * recall2stem / \
            (precision2stem + recall2stem)
    ...
    if (precision3stem + recall3stem) > 0:
        f3stem = 2 * precision3stem * recall3stem / \
            (precision3stem + recall3stem)
...
def readInData(filename):
    ...
    (trendid, trendname, origsent, candsent, judge, origsenttag,
     candsenttag) = (None, None, None, None, None, None, None)

    for line in open(filename):
        line = line.strip()
        # read in training or dev data with labels
        if len(line.split('\t')) == 7:
            (trendid, trendname, origsent, candsent, judge,
             origsenttag, candsenttag) = line.split('\t')
        # read in test data without labels
        elif len(line.split('\t')) == 6:
            (trendid, trendname, origsent, candsent,
             origsenttag, candsenttag) = line.split('\t')
        else:
            continue
        ...
        if judge[0] == '(':  # labelled by Amazon Mechanical Turk in format like "(2,3)"
            ...
            if nYes >= 4:
                expert_label = True
                data.append((features, expert_label,
                            origsent, candsent, trendid))
            elif nYes <= 2:
                expert_label = False
                data.append((features, expert_label,
                            origsent, candsent, trendid))
...
def OneEvaluation():
    ...
    # train the model
    classifier = nltk.classify.maxent.train_maxent_classifier_with_megam(
        train, gaussian_prior_sigma=10, bernoulli=True)
    ...
    for i, t in enumerate(test):
        ...
        if guess == True:
            print("GOLD-" + str(label) + "\t" + "SYS-" +
                  str(guess) + "\t" + sent1 + "\t" + sent2)
...
def PREvaluation():
    ...
    sortedindex = sorted(list(range(len(probs))), key=probs.__getitem__)
    sortedindex.reverse()
    ...
    for sortedi in sortedindex:
        ...
        print(str(i) + "\t" + strhit + "\t" + "{0:.3f}".format(
            precision) + '\t' + "{0:.3f}".format(recall) + "\t" + "{0:.3f}".format(f1), end=' ')
        print("\t|||\t" +
              "{0:.3f}".format(probs[sortedi]) + "\t" + sent1 + "\t" + sent2)
